...
- Omarchy plugin panel (Play / Host / Join / edition picker) + menu + `.desktop`
...
That clones vkQuake 1.35 into `vendor/vkQuake` if missing, compiles it, and installs the launcher, Quickshell plugin, and Hyprland rules. The engine is not in git. Do **not** copy a `build/` directory from another machine — Meson bakes in absolute paths. First Play downloads shareware.

If `make install` still mentions `/home/ryanr/repos2026/quake-omarchy`, delete `build/` and run it again.
...
| `vendor/vkQuake` | Engine source (vkQuake 1.35.0) |
